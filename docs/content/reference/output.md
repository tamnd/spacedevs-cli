...
describes what users get. Pick a format with `-o`, or let spacedevs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
spacedevs <command> -o table   # aligned columns for reading
spacedevs <command> -o jsonl   # one JSON object per line, for piping
spacedevs <command> -o json    # a single JSON array
spacedevs <command> -o csv     # spreadsheet friendly
spacedevs <command> -o tsv     # tab-separated
spacedevs <command> -o url     # just the URL column
spacedevs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
spacedevs <command> --fields id,title,url
```
...
```bash
spacedevs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
spacedevs <command>            # a table, because this is a terminal
spacedevs <command> | wc -l    # JSONL, because this is a pipe
```
